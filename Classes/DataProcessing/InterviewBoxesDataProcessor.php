...
final class InterviewBoxesDataProcessor implements DataProcessorInterface
{
    /**
    ...
    public function process(
        ...
        foreach ($rows as $row) {
            $uid = (int)$row['uid'];
            $videoUrl = $this->getFirstFileUrl($connectionPool, $resourceFactory, $uid, 'media_video');
            $vimeoEmbedUrl = $this->toVimeoEmbedUrl($videoUrl);

            $interviews[] = [
                'uid' => $uid,
                'name' => trim((string)($row['title'] ?? '')),
                'subline' => trim((string)($row['subline'] ?? '')),
                'statement' => (string)($row['bodytext'] ?? ''),
                'videoUrl' => $videoUrl,
        ...
        $processedData['interviews'] = $interviews;
    ...
    private function fetchRows(ConnectionPool $connectionPool, int $contentElementUid, int $languageId): array
    {
        ...
        $queryBuilder = $connectionPool->getQueryBuilderForTable('tx_aistealpproductslider_interview');
        $queryBuilder->getRestrictions()->removeAll();
        $queryBuilder->getRestrictions()->add(new FrontendRestrictionContainer());

        $rows = $queryBuilder
            ->select('*')
            ->from('tx_aistealpproductslider_interview')
            ->where(
    ...
    private function getFirstFileUrl(
        ...
        $row = $queryBuilder
            ...
            ->where(
                $queryBuilder->expr()->eq('tablenames', $queryBuilder->createNamedParameter('tx_aistealpproductslider_interview')),
                $queryBuilder->expr()->eq('fieldname', $queryBuilder->createNamedParameter($fieldName)),
                $queryBuilder->expr()->eq('uid_foreign', $queryBuilder->createNamedParameter($interviewUid, ParameterType::INTEGER)),
